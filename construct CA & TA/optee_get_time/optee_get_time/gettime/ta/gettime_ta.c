...
static TEE_Result ree_time(uint32_t param_types,
	TEE_Param params[4])
{
	...
	IMSG("Finish: %u", params[0].value.a, params[0].value.b);

	return TEE_SUCCESS;
}

static TEE_Result tee_time(uint32_t param_types,
	TEE_Param params[4])
{
	uint32_t exp_param_types = TEE_PARAM_TYPES(TEE_PARAM_TYPE_VALUE_INOUT,
						   TEE_PARAM_TYPE_NONE,
			...
			uint32_t param_types, TEE_Param params[4])
{
	...
	case TA_CMD_GET_REE_TIME:
		return ree_time(param_types, params);
	case TA_CMD_GET_SYSTEM_TIME:
		return tee_time(param_types, params);
	default:
		return TEE_ERROR_BAD_PARAMETERS;
	}
}
